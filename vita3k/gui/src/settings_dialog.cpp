...
void draw_settings_dialog(GuiState &gui, EmuEnvState &emuenv) {
    ...
    if (ImGui::BeginTabItem(lang.emulator["title"].c_str())) {
        ...
        if (!emuenv.cfg.tiltsens){
            ...
            ImGui::RadioButton("0 " + lang.emulator["sensor_degrees"].c_str(), &emuenv.cfg.tiltpos, 0);
            ImGui::RadioButton("90 " + lang.emulator["sensor_degrees"].c_str(), &emuenv.cfg.tiltpos, 1);
            ImGui::RadioButton("-90 " + lang.emulator["sensor_degrees"].c_str(), &emuenv.cfg.tiltpos, -1);
            config::serialize_config(emuenv.cfg, emuenv.cfg.config_path);
        }
        ImGui::Spacing();
